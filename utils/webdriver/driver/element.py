...
class Element(ElementInterface):
    ...
    def move_to_element_center_view(self, time: float = 1) -> None:
        """Move to element"""
        logger.info(
            "Element.move_to_element() - Перевести фокус в центральную точку элемента"
        )
        ...
        self._page.webdriver.execute_script(script, element)
        sleep(time)
